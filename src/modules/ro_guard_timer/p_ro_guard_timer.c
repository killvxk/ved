...
void ro_data_init(struct ro_data* ro_data) {
    ...
    if (!ro_data->mmap_min_addr_p) {
        p_print_log(P_LKRG_CRIT, "ro_guard could not check mmap_min_addr\n");
    } else {
    ...
    if (!ro_data->kptr_restrict_p)
        p_print_log(P_LKRG_CRIT, "ro_guard could not check kptr_restrict\n");
    else
    ...
    if (!ro_data->kprobes_all_disarmed_p)
        p_print_log(P_LKRG_CRIT, "ro_guard could not check kprobe_all_disarmed\n");
    else
...
void global_ro_data_check(struct ro_data* ro_data) {
    ...
    && *ro_data->mmap_min_addr_p != ro_data->mmap_min_addr)
        p_print_log(P_LKRG_CRIT, "mmap_min_addr may be corrupted\n");

#if LINUX_VERSION_CODE < KERNEL_VERSION(3,0,0)
    if (ro_data->kptr_restrict_p
    && *ro_data->kptr_restrict_p != ro_data->kptr_restrict)
        p_print_log(P_LKRG_CRIT, "kptr_restrict may be corrupted\n");
#endif

    if (ro_data->kprobes_all_disarmed_p
     && *ro_data->kprobes_all_disarmed_p != ro_data->kprobes_all_disarmed) {
        p_print_log(P_LKRG_CRIT, "all kprobe was disable, VED was disarmed\n");
        p_offload_work(0);
    }
...
void kprobe_check(void) {
    ...
    p_kprobes_disabled_warn(P_LKRG_INFO);
